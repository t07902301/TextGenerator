...
class FakeTextDataGenerator(object):
    ...
    def generate(
        ...
        margin_top, margin_left, margin_bottom, margin_right = rnd.randint(1, margin_top), rnd.randint(1, margin_left), rnd.randint(1, margin_bottom), rnd.randint(1, margin_right)
        ...
        # 㓱
        for ch in text:
            if not judge_char_in_font1(ch, font, font_dict[font]):
                return image
            if not judge_char_in_font2(ch, font, font_dict[font]):
                return image
            #加一个图像方法的判断
            if not computer_text_generator.judge_normal(ch, font):
                return image
        if is_handwritten:
            if orientation == 1:
        ...
        if background_type == 0:
            ...
            background_img = background_generator.add_background(
                background_height, background_width, image_list,index
            )#从图片中获取背景
        else:
            background_img = background_generator.gen_rand_bg(
                background_height, background_width
            )#随机背景
        ...
        #######################        
        if light_degree!=0:
            background_img=background_generator.shadow_light(background_img,light_degree)

        #######################
        # Apply noise #
        #######################  
        background_img=background_generator.add_noise(noise_type,background_img)
        ...
        if name_format == 0:
            ...
            image_name = "{}_{}.{}".format(name_pre, str(index), extension)
        ...
        if out_dir is not None:
            ...
            # return final_image,os.path.join(image_dir,images[index%35])
