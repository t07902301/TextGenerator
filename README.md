...
<font size=3>A modified synthetic data generator for text recognition based on [this repository](https://github.com/Belval/TextRecognitionDataGenerator)</font>
...
Clone the repository and simply run `pip install -r requirements.txt` to install dependencies.

Note that only Python interpreters with version **less than 3.9** can work, because  `Pillow` cannot support the latest release of Python. 

Here a <font size=3>[**virtualenv**](https://zhuanlan.zhihu.com/p/38457004)</font> is highly recommended.
    

#### **运行例子**: 
* 在trdg目录下
```
python run.py -l en -c 20000 -w 3 -na 2 -npre num_syn1 -rs -k 1 -rk -b 3 -num --output_dir C:/DM_hcn/data/ocr/math_ch_print/img/num_syn/num_syn1
```

## Examples of functions
For details on usage of different arguments, please refer to `parse_arguments` function in `run.py`.
### 1. Text skewing

What if you want random skewing? Add `-k` and `-rk` (` -c 1000 -w 5 -f 64 -k 5 -rk`)
...
Maybe you want another background? Add `-b` to define one of the three available backgrounds: plain white (0), quasicrystal (1) or image (2).
...
### 5. Noise
`-ns 1` , where 1 stands for the type of noise to add.
...
### 6. Light
`-ld 100` , where 100 stands for the range of degree of light added to images.

![32](samples/32.jpg "0")

### 7. Mask
If you set `-ms True` , mask will be added to images. 

![29](samples/29.jpg "0")

### 8. Create images with Chinese text
...
## New
- Add `--noise` argument to determine what kind of noise is to be added to images
- Add `--light_degree` argument to set the degree of light added to images
- Modify `--background` argument to specify only three types of background 
